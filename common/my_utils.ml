...
module L     = List
module Mar   = Legacy.Marshal
module P     = Legacy.Printf
module RNG   = Random
module S     = String
...
(* WARNING: potentially inefficient
   (all lines are option-wrapped in the process) *)
let line_stream_of_file fn =
  let reader_fun input _i =
    try Some (Legacy.input_line input)
    with End_of_file ->
      let _ = Legacy.close_in input in
      None
  in
  let input = Legacy.open_in fn in
  Stream.from (reader_fun input)

let list_to_file f to_string l =
  let out = Legacy.open_out f in
  L.iter
    (fun elt -> P.fprintf out "%s" (to_string elt))
    l;
  Legacy.close_out out
...
let iter_on_lines_of_file f file =
  let input = Legacy.open_in file in
  try
    while true do
      f (Legacy.input_line input)
    done
  with End_of_file -> Legacy.close_in input
...
(* fold_left f on l while (p x) is true *)
let fold_while f p lst =
...
let nsplit p li =
  let rec loop local_acc global_acc = function
    | []      -> L.rev ((L.rev local_acc) :: global_acc)
    | l :: ls ->
      if p l then
        loop [] ((L.rev local_acc) :: global_acc) ls
      else
        loop (l :: local_acc) global_acc ls
  in
  loop [] [] li
...
let covariance xs ys =
  let count, sum_xy, sum_x, sum_y =
    L.fold_left2
      (fun (c, sxy, sx, sy) x y -> (c + 1, sxy +. (x *. y), sx +. x, sy +. y))
      (0, 0.0, 0.0, 0.0)
      xs ys
  in
  let n = float_of_int count in
  (sum_xy -. ((sum_x *. sum_y) /. n)) /. n
...
let manhattan_dist xs ys =
  L.fold_left2
    (fun acc x y -> acc +. abs_float(x -. y))
    0.0
    xs ys

(* formula comes from doi={10.1023/B:JCAM.0000017375.61558.ad} *)
let euclidian_dist xs ys =
  let sum_of_squares =
    L.fold_left2
      (fun acc x y -> let delta = x -. y in
                      acc +. (delta *. delta))
      0.0
      xs ys
  in sqrt(sum_of_squares)
...
let cross_corr_l xs ys =
  let x_avg = average_l xs in
  let y_avg = average_l ys in
  L.fold_left2
    (fun acc x y -> acc +. ((x -. x_avg) *. (y -. y_avg)))
    0.0
    xs ys
...
let ks_l l1 l2 =
  let to_map l =
    L.fold_left
      (fun acc (d, cdf) -> HT.add acc d cdf; acc)
      (HT.create 1000)
      l
  in
  let map1 = to_map l1 in (* d1 to cdf1 *)
  let map2 = to_map l2 in (* d2 to cdf2 *)
  let inflection_points =
    (* FloatSet prevents duplication of values *)
    FloatSet.elements
      (FloatSet.of_list
         (L.rev_append
            (L.rev_map fst l1)
            (L.rev_map fst l2)))
  in
  (* duplicate values to have all the inflection points *)
  let rebuild map =
    L.rev
      (snd
         (L.fold_left
            (fun (prev_cdf, acc) d ->
               let curr_cdf = find_with_default 0.0 map d in
               let new_cdf = max curr_cdf prev_cdf in
               (new_cdf, (d, new_cdf) :: acc)
            )
            (0.0, [])
            inflection_points))
  in
  let l1' = rebuild map1 in
  let l2' = rebuild map2 in
  (* let to_string (x, y) = *)
  (*   sprintf "%f %f\n" x y *)
  (* in *)
  (* list_to_file "l1p.cdf" to_string l1'; *)
  (* list_to_file "l2p.cdf" to_string l2'; *)
  (* find the max dist between CDFs *)
  L.fold_left2
    (fun prev_max (d1, cdf1) (d2, cdf2) ->
       if d1 <> d2 then
         failwith "my_utils.ml: ks_l: different d found"
       else
         let curr = abs_float (cdf1 -. cdf2) in
         max curr prev_max
    )
    0.0
    l1' l2'
...
(* Kuiper's test *)
let kui_l _l1 _l2 =
  failwith "kui_l: not implemented yet"
...
let carbo l1 l2 =
  fst (carbo_hodgkin l1 l2)

let hodgkin l1 l2 =
  snd (carbo_hodgkin l1 l2)
...
let get_command_output cmd =
  let cmd_out   = Legacy.Unix.open_process_in cmd in
  let buff_size = 1024*1024                       in
  let buff      = Legacy.Buffer.create buff_size  in
  let _         =
    try
      while true do
        Legacy.Buffer.add_string buff (Legacy.input_line cmd_out);
        Legacy.Buffer.add_char   buff '\n'; (* was stripped by input_line *)
      done;
    with
    | End_of_file -> ignore(Legacy.Unix.close_process_in cmd_out)
    | exn         -> ignore(Legacy.Unix.close_process_in cmd_out); raise exn
  in
  Legacy.Buffer.contents buff
